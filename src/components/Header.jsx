...
export default function Header({ onCartClick, cartCount }) {
  return (
    <header className="max-w-5xl mx-auto bg-white shadow-lg rounded-2xl p-6 flex items-center gap-6">
      <img
      ...
      <div>
        <button
          onClick={onCartClick}
          className="relative bg-rose-500 hover:bg-rose-600 text-white px-4 py-2 rounded-lg shadow"
        >
          Carrinho
          <span className="absolute -top-2 -right-2 bg-white text-rose-600 rounded-full w-6 h-6 flex items-center justify-center font-semibold">
            {cartCount}
          </span>
        </button>
      </div>
